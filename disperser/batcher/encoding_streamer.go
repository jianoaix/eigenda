...
func (e *EncodingStreamer) RequestEncoding(ctx context.Context, encoderChan chan EncodingResultOrStatus) error {
	...
	for i := range metadatas {
		...
		if float64(time.Since(requestTime).Milliseconds()) > 2000 {
			e.logger.Info("XXX encoding_requested age - picked up blobmetadata", "age", time.Since(requestTime).String(), "blob size bytes", metadata.RequestMetadata.BlobSize, "blob key", metadata.GetBlobKey().String())
		}
	}
	...
	for i := range metadatas {
		...
		if float64(time.Since(requestTime).Milliseconds()) > 2000 {
			e.logger.Info("encoding_requested age - after fetched blobs", "XXX age", time.Since(requestTime).String(), "blob size bytes", metadata.RequestMetadata.BlobSize, "blob key", metadata.GetBlobKey().String())
		}
...
func (e *EncodingStreamer) RequestEncodingForBlob(ctx context.Context, metadata *disperser.BlobMetadata, blob *core.Blob, state *core.IndexedOperatorState, referenceBlockNumber uint, encoderChan chan EncodingResultOrStatus) {
	...
	if len(pending) > 0 {
		...
		if float64(time.Since(requestTime).Milliseconds()) > 2000 {
			e.logger.Info("encoding_requested age - before sending requests", "XX big latency found", time.Since(requestTime).String(), "blob size bytes", metadata.RequestMetadata.BlobSize, "blob key", metadata.GetBlobKey().String())
		}
